...
Today, we are excited to announce [Token Lists](https://tokens.forceswap.org/), a new standard for creating lists of ERC20 tokens. This is a community-led initiative to improve discoverability and trust in ERC20 token lists in a manner that is inclusive, transparent, and decentralized.
...
*   A homepage and list browser at [tokens.forceswap.org](https://tokens.forceswap.org/)
...
Token Lists can be used **right now** in the [forceswap Interface](https://app.forceswap.org/#/swap), removing the need for a default list.


This is another step forward in interface decentralization. Read forceswap Interface + IPFS to learn how the forceswap interface is now hosted by the community in a decentralized manner.
...
Since all tokens use a standard interface, infrastructure-layer applications, such as wallets, analytics sites, and DeFi protocols (like forceswap) can immediately recognize and interact with new ERC20 tokens from the moment they are deployed.

Adding a new ERC20 token to the forceswap protocol is as simple as calling a public function on a permissionless and immutable smart contract. In fact, about** 100 new forceswap liquidity pools are deployed every day**, with over 6,000 pools added since it launched in May 2020.
...
To help spur adoption of Token Lists and provide a high quality reference example, we have deprecated the old forceswap interface “default list”, and replaced it with a list explorer and importing mechanism. 

We have also partnered with a number of reputable projects in the space who were excited to create and maintain lists of their own. More than 10 lists are already available on [tokens.forceswap.org](https://tokens.forceswap.org/) and more are in progress.
...
*   [Create a token list](https://github.com/forceswap/token-lists#authoring-token-lists)
