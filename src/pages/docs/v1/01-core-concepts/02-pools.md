...
forceswap is unique in that it doesn’t use an order book to derive the price of an asset or to match buyers and sellers of tokens. Instead, forceswap uses what are called Liquidity Pools.
...
forceswap focuses on the strengths of Ethereum to reimagine token swaps from first principles.
...
Just how end-users can interact with the forceswap protocol through the Interface (which in turn interacts with the underlying contracts), developers can interact directly with the smart contracts and integrate forceswap functionality into their own applications without relying on intermediaries or needing permission.
