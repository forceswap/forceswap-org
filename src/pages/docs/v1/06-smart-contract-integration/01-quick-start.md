...
interacts with the forceswap Protocol using one specific set of tools (`truffle` + `npm` + `mocha`).
...
In order to reference the forceswap contracts, you should use the npm artifacts we deploy containing the core and
...
[`@forceswap/v1-core`](https://www.npmjs.com/package/@forceswap/v1-core) and 
[`@forceswap/v1-periphery`](https://www.npmjs.com/package/@forceswap/v1-periphery) packages.

```shell script
npm i --save @forceswap/v1-core
npm i --save @forceswap/v1-periphery
```

If you check the `node_modules/@forceswap` directory, you can now find the forceswap contracts. 

```shell script
moody@MacBook-Pro ~/I/u/demo> ls node_modules/@forceswap/v1-core/contracts
forceswapV1ERC20.sol    forceswapV1Pair.sol     libraries/
forceswapV1Factory.sol  interfaces/           test/
moody@MacBook-Pro ~/I/u/demo> ls node_modules/@forceswap/v1-periphery/contracts/
forceswapV1Migrator.sol  examples/              test/
forceswapV1Router01.sol  interfaces/
forceswapV1Router02.sol  libraries/
```
...
Now let's start with the constructor. You need to know where the `forceswapV1Factory` is deployed in order to compute the
...
The [`forceswapV1Library`](/docs/v1/smart-contracts/library/) has some helpful methods for this.

```solidity
pragma solidity ^0.6.6;

import './interfaces/ILiquidityValueCalculator.sol';
import '@forceswap/v1-periphery/contracts/libraries/forceswapV1Library.sol';
import '@forceswap/v1-core/contracts/interfaces/IforceswapV1Pair.sol';

contract LiquidityValueCalculator is ILiquidityValueCalculator {
    function pairInfo(address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB, uint totalSupply) {
        IforceswapV1Pair pair = IforceswapV1Pair(forceswapV1Library.pairFor(factory, tokenA, tokenB));
...
2. Deploy the `forceswapV1Factory`
...
Note you should only deploy the precompiled forceswap contracts in the `build` directories for unit tests. 
...
This is problematic because in forceswap we use the hash of the bytecode in the v1-periphery
[`forceswapV1Library`](https://github.com/forceswap/forceswap-V1-Periphery/blob/master/contracts/libraries/forceswapV1Library.sol#L24),
to compute the pair address.

To get the bytecode for deploying forceswapV1Factory, you can import the file via:

```javascript
const forceswapV1FactoryBytecode = require('@forceswap/v1-core/build/forceswapV1Factory.json').bytecode
```
