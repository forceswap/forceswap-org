...
const APOLLO_QUERY = gql`
  {
    forceswapFactory(id: "0xdD9EFCbDf9f422e2fc159eFe77aDD3730d48056d") {
      totalVolumeUSD
      totalLiquidityUSD
      pairCount
    }
...
export const forceSWAP_GLOBALS_24HOURS_AGO_QUERY = block => {
  let queryString = `
  query forceswapFactory {
    forceswapFactory(id: "0xdD9EFCbDf9f422e2fc159eFe77aDD3730d48056d", block: { number: ${block} }) {
      totalVolumeUSD
      totalLiquidityUSD
      pairCount
...
const About = props => {
  ...
  useEffect(() => {
    async function getData() {
      let result = await client.query({
        query: forceSWAP_GLOBALS_24HOURS_AGO_QUERY(oneDayBackBlock),

        fetchPolicy: 'cache-first'
      })
      if (result) {
        setOnedayResult(result?.data?.forceswapFactory)
      }
    }
    if (oneDayBackBlock) {
      getData()
  ...
  if (data && oneDayResult) {
    const volume24Hour = parseFloat(data?.forceswapFactory?.totalVolumeUSD) - parseFloat(oneDayResult?.totalVolumeUSD)
    ...
    forceStats.liquidity = [
      ...
      }).format(data.forceswapFactory.totalLiquidityUSD)
    ]
    forceStats.exchanges = [Number.parseFloat(data?.forceswapFactory?.pairCount)]
      ...
      <StyledAbout>
        <span style={{ marginTop: '5rem' }}>
          ...
          <StyledSectionFlex id="about" style={{ flexDirection: 'column' }}>
            <p>
              forceswap empowers developers, liquidity providers and traders to participate in a financial marketplace
          ...
          <StyledSectionFlex id="contact" style={{ flexDirection: 'column' }}>
            ...
            <p>
              To get in touch, please email <a href="mailto:admin@forceswap.org">admin@forceswap.org</a>
            </p>

            <p>
              We encourage anyone facing issues with their wallet, transaction or forceswap related question to join our
          ...
          <StyledSectionFlex id="jobs" style={{ flexDirection: 'column' }}>
            ...
            <p>
              forceswap welcomes all qualified persons to apply regardless of race, religion, gender, gender identity or
